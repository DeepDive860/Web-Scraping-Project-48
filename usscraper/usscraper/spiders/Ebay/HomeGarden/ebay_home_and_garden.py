...
class Ebay_Home_And_Garden(scrapy.Spider):

    name = "ebay_spider_home_and_garden_bath"
    ...
    async def parse_data(self, response):
        ...
        try:
            ...
            products = response.css("li.brwrvr__item-card")
            for product in products:
                image_urls = response.css("img.brvrvr__item-card__image::attr(src)").get()

                rating_url = response.css("a.bsig__product-review::attr(href)").get()
                    ...
                    yield scrapy.Request(
                        ...
                        meta={
                            ...
                            "playwright_page_methods": [
                                ...
                                PageMethod("wait_for_selector", "div.right-summary-panel-container.vi-mast__col-right", timeout=120000),
                                PageMethod("wait_for_selector", "div.tabs__content", timeout=120000),
    ...
    async def parse_item(self, response):
        ...
        try:
            ...
            print("\n")
            item["product_id"] = response.css("div.ux-labels-values col-6.ux-labels-values__column-last-row.ux-labels-values--ebayProductId(epid) span.ux-textspans::text").get()
            ...
            if not item["original_price"]:
                item["original_price"] = response.css("span.ux-textspans.ux-textspans--STRIKETHROUGH::text").get()
            item["discount_percent"] = response.css("span.ux-textspans.ux-textspans--EMPHASIS::text").get()
            if not item["discount_percent"]:
                item["discount_percent"] = response.css("span.ux-textspans.ux-textspans--SECONDARY::text").get()
            item["category"] = " > ".join(response.css("a.seo-breadcrumb-text::text").getall())
            
            review_url = response.meta.get("rating_url")
            ...
            if review_url:
                yield scrapy.Request(
                    ...
                    meta={
                        ...
                        "item": item,
                        "playwright_page_methods": [
                            PageMethod("wait_for_selector", "div.vim.x-star-rating", timeout=120000),
                            PageMethod("wait_for_selector", "a.x-star-rating__scrollToReviews.ux-action", timeout=120000)
                        ],
                        "playwright_context_kwargs": {
                            "user_agent": random.choice(USER_AGENTS)
                        } 
            ...
            item["condition"] = response.css("div.vim x-item-condition.mar-t-20.x-item-condition_aligned span.ux-textspans::text").get()
            item["units_sold"] = response.css("div#qtyAvailability span.ux-textspans.ux-textspans--BOLD ux-textspans--EMPHASIS::text").get()
            item["number_watching"] = response.css("div.ux-section-icon-with-details__data-item-text span.ux-textspans::text").get()
            item["product_url"] = response.meta.get("product_url")
            item["image_url"] = response.meta.get("image_urls")
            if "US" in item["current_price"]:
                item["currency"] = "USD"
    ...
    async def parse_ratings(self, response):
        ...
        try:
            ...
            item["ratings"] = response.css("div.vim.x-star-rating span.ux-textspans::text").get()
            item["review_count"] = response.css("a.x-star-rating__scrollToReviews.ux-action span.ux-textspans.ux-textspans--PSEUDOLINK::text").get()
            yield item
